...
function setupTongsRig(rawScene) {
  ...
  const nodeA1 = rawScene.getObjectByName('Box006');
  const nodeA2 = rawScene.getObjectByName('Box007');
  const nodeB1 = rawScene.getObjectByName('Box008');
  const nodeB2 = rawScene.getObjectByName('Box009');
  const pivotNodes = ['Box010', 'Cylinder002'].map((n) => rawScene.getObjectByName(n)).filter(Boolean);
  ...
  if (nodeA1 && nodeA2 && nodeB1 && nodeB2 && pivotNodes.length > 0) {
    const pivotWorld = new THREE.Vector3();
    pivotNodes.forEach((n) => { const p = new THREE.Vector3(); n.getWorldPosition(p); pivotWorld.add(p); });
    pivotWorld.divideScalar(pivotNodes.length);

    const armA = new THREE.Group();
    const armB = new THREE.Group();
    const pivotG = new THREE.Group();
    rig.add(armA, armB, pivotG);
    scene.add(rig);
    armA.position.copy(pivotWorld);
    armB.position.copy(pivotWorld);
    pivotG.position.copy(pivotWorld);
    rig.updateMatrixWorld(true);

    armA.attach(nodeA1); armA.attach(nodeA2);
    armB.attach(nodeB1); armB.attach(nodeB2);
    pivotNodes.forEach((n) => pivotG.attach(n));

    rig.userData.armA = armA;
    rig.userData.armB = armB;
    scene.remove(rig);
  } else {
    scene.remove(rawScene);
    rig.add(rawScene);
  }

  rig.traverse((c) => { if (c.isMesh) { c.castShadow = true; c.receiveShadow = true; } });
  rig.position.set(0.92, 1.09, -1.05);
  scene.add(rig);
  grabbables.push(rig);
  showToast('🥢 Maşa hazır!');
}
...
function updateTongs(delta) {
  grabbables.forEach((obj) => {
    ...
    obj.userData.closeAmount = THREE.MathUtils.lerp(obj.userData.closeAmount, target, Math.min(1, delta * 10));
    if (obj.userData.armA && obj.userData.armB) {
      const t = obj.userData.closeAmount;
      obj.userData.armA.rotation.y = -TONG_CLOSE_ANGLE * t;
      obj.userData.armB.rotation.y = TONG_CLOSE_ANGLE * t;
    }
    ...
    if (isHeld && obj.userData.closed && !obj.userData.heldKofte && obj.userData.closeAmount > 0.8) {
      const pos = new THREE.Vector3();
      obj.getWorldPosition(pos);
      let nearest = null, nd = 0.16;
...
function loadGloves() {
  gltfLoader.load(
    ...
    (gltf) => {
      ...
      // İki eldiven oluştur (sol ve sağ kontrolcü için)
      makeGloveFor('left', controller1, new THREE.Vector3(0, 0.04, 0.07));
      makeGloveFor('right', controller2, new THREE.Vector3(0, 0.04, 0.07));
      // İlk frame'de pozisyonları ayarla (henüz controller bağlı değilken sahnede dursun)
      gloves.forEach(g => {
...
function makeGloveFor(handedness, controller, attachOffset) {
  ...
  // Her eldiven için modeli klonla (parmak pivot'ları bağımsız animasyon için)
  const instance = gloveModel.clone(true);
  instance.traverse((c) => {
    if (c.isMesh) { c.castShadow = true; c.receiveShadow = true; }
  });

  // Parmak pivot node'larını bul (her eldiven için)
  const pivotNodes = {};
  instance.traverse((c) => {
    if (c.isObject3D && c.name) {
      if (['Index', 'Middle', 'Ring', 'Pinky', 'Thumb'].includes(c.name)) {
        pivotNodes[c.name] = c;
        // Başlangıç rotasyonunu sakla
  ...
  // Sağ el için: X ekseninde çevir (solda işaret parmağı +X'te, sağda -X'te)
  if (handedness === 'right') {
    instance.scale.x = -1;
  }

  // Eldiven boyutunu biraz küçült (kontrolcüye orantılı)
  instance.scale.multiplyScalar(0.85);

  // playerRig'e ekle (kamera ile birlikte döner)
  ...
  gloves.push({
    ...
    handedness,
    attachOffset,
    closeAmount: 0, // 0 = açık, 1 = kapalı (sıkılı yumruk)
    targetClose: 0,
...
function updateGloves(delta) {
  ...
  gloves.forEach((g) => {
    if (inVR && g.controller) {
      // VR'de kontrolcüye bağla
      g.instance.visible = true;
      // Pozisyon: kontrolcünün hemen önünde (parmaklar ileri, avuç içi kontrolcüye bakacak)
      // Kontrolcünün doğal "ileri" yönü -Z'dir
      g.controller.add(g.instance);
      g.instance.position.copy(g.attachOffset);
      // Parmak avuç içi yukarı, kontrolcüye doğru
      g.instance.rotation.set(-Math.PI / 2, 0, g.handedness === 'left' ? 0.15 : -0.15);
    } else if (inVR) {
      g.instance.visible = false;
    } else {
      // Masaüstünde: controller atanmamışsa (henüz bağlanmadıysa) sahnede bırak
      // ama VR'de değilsek görünmesin - mouse ile eldiven göstermek overkill olur
      g.instance.visible = false;
    }

    // Hedef: sıkılıyorsa (grip basılı / Boşluk + eldiven bağlı) kapan
    let target = 0;
    if (g.controller) {
      const held = g.controller.userData.grabbed;
      const gripPressed = g.controller.userData.gripPressed;
      // Kontrolcü bir şey tutuyorsa veya grip basılıysa parmaklar kapan
    ...
    Object.entries(g.pivotNodes).forEach(([name, node]) => {
      ...
      // Parmakları kapat (base rotasyon üzerine -X yönünde bük)
